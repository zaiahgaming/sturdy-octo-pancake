...
function announcePowerup(message) {
    const announcer = document.getElementById('powerup-announcements');
    if (announcer) {
    ...
    powerups.forEach((powerup, index) => {
        ...
        if (distSq < radii * radii) {
            ...
            if (powerup.type === 'spread') {
                ...
                announcePowerup('Spread Shot powerup acquired');
            }
            if (powerup.type === 'rapid') {
                activePowerups.rapid = 600;
                announcePowerup('Rapid Fire powerup acquired');
            }
            if (powerup.type === 'shield') {
                activePowerups.shield = true;
                announcePowerup('Shield powerup acquired');
            }
...
function updatePowerupTimers() {
    ...
    if (activePowerups.spread > 0) {
        ...
        if (activePowerups.spread === 0) announcePowerup('Spread Shot expired');
        if (activePowerups.spread % 60 === 0) uiNeedsUpdate = true;
    }
    if (activePowerups.rapid > 0) {
        activePowerups.rapid--;
        if (activePowerups.rapid === 0) announcePowerup('Rapid Fire expired');
