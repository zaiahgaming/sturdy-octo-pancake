...
**Action:** Always use squared distance calculations (`dx*dx + dy*dy < radius*radius`) in hot paths for logic checks. If true distance is required for vector normalization, prefer `Math.sqrt(dx*dx + dy*dy)` over `Math.hypot` as it evaluates slightly faster in most JS engines.## 2026-07-14 - Vector Normalization Performance
