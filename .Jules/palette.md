...
## 2024-11-20 - ARIA Live Announcements
**Learning:** Placing `aria-live="polite"` on frequently updating UI elements (like countdown timers) causes screen reader spam and disrupts the user experience.
**Action:** Used `aria-hidden="true"` on the frequently updating container and announced meaningful state changes using a visually hidden, dedicated `aria-live` region that clears its text content after a brief delay.
