...
## 2024-05-24 - Screen Reader Spam on Timers
**Learning:** For accessibility, avoid placing `aria-live="polite"` on frequently updating UI elements (like countdown timers) to prevent screen reader spam.
**Action:** Instead, use `aria-hidden="true"` on the frequently updating container and announce meaningful state changes using a visually hidden, dedicated `aria-live` region, clearing its textContent after a delay.
